...
class PaymentHandler: NSObject {
    ...
    var paymentSymmaryItems = [PKPaymentSummaryItem]()
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            ...
            shippingDelivery.detail = "Supplements are sent to your address"
    ...
    func startPayment(products: [Product], total: Int, completion: @escaping PaymentCompletionHandler) {
        ...
        products.forEach { product in
            let item = PKPaymentSummaryItem(label: product.name, amount: NSDecimalNumber(string: "\(product.price).00"), type:
                .final)
            paymentSymmaryItems.append(item)
        }
        
        let total = PKPaymentSummaryItem(label: "Total", amount: NSDecimalNumber(string: "\(total).00"), type: .final)
        paymentSymmaryItems.append(total)
        
        let paymentRequest = PKPaymentRequest()
        paymentRequest.paymentSummaryItems = paymentSymmaryItems
        paymentRequest.merchantIdentifier = "merchant.HIPM"
